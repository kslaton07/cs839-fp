...
Results are saved to:
  - results_acrobot/metrics.json
  - results_acrobot/summary.csv
  - results_acrobot/plots/
  - weights_acrobot/
"""
...
from acrobot import AcrobotEnv, rk4, rk2, feuler, seuler, ieuler, vverlet, wrap, bound
...
class DQN:
    ...
    def save(self, path):
        """Save network weights to a .npz file for later visualization."""
        np.savez(path,
...
def train_one(integrator_name, dt, seed=SEED):
    ...
    # Save weights for this configuration
    os.makedirs("weights_acrobot", exist_ok=True)
    weight_path = f"weights_acrobot/{integrator_name}_dt{dt}.npz"
...
def main():
    ...
    try:
        ...
        for integ_name, runs in by_integrator.items():
            ...
            ax.grid(alpha=0.3)
            plt.tight_layout()
            fname = f"results_acrobot/plots/{integ_name}.png"
        ...
        for default_dt in TIMESTEPS:
            ...
            ax.grid(alpha=0.3)
            plt.tight_layout()
            fname = f"results_acrobot/plots/comparison_dt_{default_dt}.png"
            plt.savefig(fname, dpi=150)
            plt.close()
            print(f"  Plot saved: {fname}")
        ...
        plt.tight_layout()
        plt.savefig("results_acrobot/plots/heatmap_solve.png", dpi=150)
        plt.close()
        print("  Plot saved: results_acrobot/plots/heatmap_solve.png")
        ...
        for i in range(len(integrator_names)):
            for j in range(len(TIMESTEPS)):
                ...
                ax.text(j, i, txt, ha="center", va="center", fontsize=7)
        plt.tight_layout()
        plt.savefig("results_acrobot/plots/heatmap_nan.png", dpi=150)
        plt.close()
        print("  Plot saved: results_acrobot/plots/heatmap_nan.png")
        ...
        plt.tight_layout()
        plt.savefig("results_acrobot/plots/wallclock_time.png", dpi=150)
        plt.close()
        print("  Plot saved: results_acrobot/plots/wallclock_time.png")
