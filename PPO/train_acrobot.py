...
Usage:
    python ./PPO/train_PPO_acrobot.py    (run from project root)

All outputs are saved relative to this script's directory:
  - <PPO>/results_PPO_acrobot/metrics.json
  - <PPO>/results_PPO_acrobot/summary.csv
  - <PPO>/results_PPO_acrobot/plots/
  - <PPO>/weights_PPO_acrobot/

Reward scheme: reward *= dt keeps episode returns comparable across dt values.
"""
...
RESULTS_DIR = f"{SCRIPT_DIR}/results_PPO_acrobot"
PLOTS_DIR   = f"{RESULTS_DIR}/plots"
WEIGHTS_DIR = f"{SCRIPT_DIR}/weights_PPO_acrobot"
...
def train_one(integrator_name, dt, seed=SEED):
    ...
    env = AcrobotEnv(integrator=_integrators[integrator_name], dt=dt)
    ...
    for ep in range(NUM_EPISODES):
        ...
        for _ in range(MAX_STEPS):
            ...
            # Scale reward so returns are comparable across dt values.
            reward *= dt
        ...
        # Bootstrap value: 0 if goal reached / fell, else V(last obs)
...
def main():
    ...
    MAX_PARALLEL = 12
    print(f"Running {len(configs)} configurations x {NUM_EPISODES} episodes each "
          f"(PPO, Ray, max {MAX_PARALLEL} at a time).\n")
