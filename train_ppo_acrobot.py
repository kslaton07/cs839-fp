"""
PPO training script for AcrobotEnv — compares integrators and timesteps.
Uses Stable Baselines 3 PPO.

Mirrors the structure of train_acrobot.py but uses PPO instead of DQN.
Results saved to:
  - results_PPO_acrobot/metrics.json
  - results_PPO_acrobot/summary.csv
  - results_PPO_acrobot/plots/
  - weights_PPO_acrobot/   (.npz files, same format as DQN for visualize.py)

Usage:
    python train_PPO_acrobot.py
"""
...
# ─────────────────────────────────────────────────────────────────────────────
# Weight extraction helper — pulls numpy arrays out of SB3 policy
# so visualize.py can load them in the same .npz format as DQN
# ─────────────────────────────────────────────────────────────────────────────
def extract_and_save_weights(model, path):
    """
    Extract weights from SB3 PPO MlpPolicy and save as .npz.
    SB3 MlpPolicy has mlp_extractor (shared layers) + action_net (output).
    We save the three layer matrices to match DQN's W0/W1/W2 format.
    """
    params = {k: v.cpu().numpy() for k, v in model.policy.state_dict().items()}

    # SB3 MlpPolicy layer names for MLP extractor + action net
    # Typical keys: mlp_extractor.policy_net.0.weight/bias,
    ...
    try:
        ...
        # Fallback: just save whatever params exist
        np.savez(path, **{k.replace(".", "_"): v for k, v in params.items()})


# ─────────────────────────────────────────────────────────────────────────────
# Gymnasium wrapper — SB3 needs a standard gym.Env interface
# ─────────────────────────────────────────────────────────────────────────────
class AcrobotWrapper:
    """Thin wrapper to make AcrobotEnv compatible with SB3."""
    def __init__(self, integrator_fn, dt, seed=SEED):
        self.env = AcrobotEnv(integrator=integrator_fn, dt=dt)
    ...
    def reset(self, **kwargs):
        return self.env.reset(seed=kwargs.get("seed", self._seed))

    def step(self, action):
        obs, reward, terminated, truncated, info = self.env.step(action)
        reward *= self.dt  # normalise to simulated-time units
...
def train_one(integrator_name, dt, seed=SEED):
    ...
    # PPO hyperparams — n_steps scaled to one episode length
    n_steps   = MAX_STEPS
    ppo_model = PPO(
        "MlpPolicy", env,
        n_steps=n_steps,
    ...
    # Train episode by episode — collect one episode manually, then update
    for ep in range(NUM_EPISODES):
        ...
        # Train PPO on one episode worth of timesteps
        ppo_model.learn(total_timesteps=MAX_STEPS, reset_num_timesteps=False)

    wall_time = time.perf_counter() - wall_start

    # Save weights in .npz format for visualize.py compatibility
    os.makedirs("weights_PPO_acrobot", exist_ok=True)
    weight_path = f"weights_PPO_acrobot/{integrator_name}_dt{dt}.npz"
...
def main():
    ...
    os.makedirs("results_PPO_acrobot/plots", exist_ok=True)
    os.makedirs("weights_PPO_acrobot", exist_ok=True)
    all_results = []
    ...
    try:
        ...
        for integ_name, runs in by_integrator.items():
            ...
            ax.set_title(f"PPO on Acrobot — integrator: {integ_name}")
            ax.set_xlabel("Episode")
            ax.set_ylabel(f"Smoothed reward (window={SMOOTH_WINDOW})")
            ax.legend(fontsize=8)
            ax.grid(alpha=0.3)
            plt.tight_layout()
            fname = f"results_PPO_acrobot/plots/{integ_name}.png"
        ...
        for default_dt in TIMESTEPS:
            ...
            ax.grid(alpha=0.3)
            plt.tight_layout()
            fname = f"results_PPO_acrobot/plots/comparison_dt_{default_dt}.png"
            plt.savefig(fname, dpi=150)
            plt.close()
            print(f"  Plot saved: {fname}")
        ...
        plt.tight_layout()
        plt.savefig("results_PPO_acrobot/plots/heatmap_solve.png", dpi=150)
        plt.close()
        print("  Plot saved: results_PPO_acrobot/plots/heatmap_solve.png")
        ...
        for i in range(len(integrator_names)):
            for j in range(len(TIMESTEPS)):
                ...
                ax.text(j, i, txt, ha="center", va="center", fontsize=7)
        plt.tight_layout()
        plt.savefig("results_PPO_acrobot/plots/heatmap_nan.png", dpi=150)
        plt.close()
        print("  Plot saved: results_PPO_acrobot/plots/heatmap_nan.png")
        ...
        plt.tight_layout()
        plt.savefig("results_PPO_acrobot/plots/wallclock_time.png", dpi=150)
        plt.close()
        print("  Plot saved: results_PPO_acrobot/plots/wallclock_time.png")
